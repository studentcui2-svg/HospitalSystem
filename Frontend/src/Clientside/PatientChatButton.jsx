...
import { FaComments, FaTimes, FaUserMd } from "react-icons/fa";
    ...
    try {
      ...
      if (response && response.chats) {
        const total = response.chats.reduce(
          (sum, chat) => sum + (chat.unreadCount?.patient || 0),
          0,
        );
  ...
  return (
    <>
      ...
      {isOpen && (
        <ChatListModal>
          <ModalHeader>
            <HeaderTitle>My Doctor Chats</HeaderTitle>
          ...
          <ChatList>
            {loading ? (
              ...
              <EmptyState>
                No active chats yet. Visit the Doctors page to start chatting!
              </EmptyState>
            ) : (
              chats.map((chat) => (
                <ChatItem key={chat._id} onClick={() => handleChatClick(chat)}>
                  ...
                  <ChatInfo>
                    <ChatName>Dr. {chat.doctor?.name || "Doctor"}</ChatName>
                    <DoctorDept>
                      {chat.doctor?.department || "Medical Professional"}
                    </DoctorDept>
                    <LastMessage>
                  ...
                  <ChatMeta>
                    ...
                    {chat.unreadCount?.patient > 0 && (
                      <UnreadBadge>{chat.unreadCount.patient}</UnreadBadge>
                    )}
                  </ChatMeta>
                </ChatItem>
              ))
      ...
      {selectedChat && (
        <ChatInterface
          ...
          doctorName={selectedChat.doctor?.name}
          doctorDepartment={selectedChat.doctor?.department}
          userRole="user"
        />
      )}
    </>
